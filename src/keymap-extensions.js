  ...
  try {
    ...
    if (error.message.indexOf('Unable to watch path') > -1) {
      const message = `Unable to watch path: \`${path.basename(userKeymapPath)}\`. Make sure you \
        ...
        [watches]:https://docs.pulsar-edit.dev/atom-archive/hacking-atom/#typeerror-unable-to-watch-path`; //TODO: Update the above to Pulsar docs if we choose to add this
